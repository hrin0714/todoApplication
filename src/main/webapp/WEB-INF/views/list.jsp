...
function goPage(pageNo){
	...
	$.ajax({
        ...
        dataType : 'json',
        cache    : false,
        success  : function( json, header, request ) {
        	//$('#temp').html(JSON.stringify(request));
        	//alert("json : "+JSON.stringify(json));
        	//alert("header : "+JSON.stringify(header));
        	//alert("totalCount : "+request.getResponseHeader('X-list-total-count'));
        ...
        error    : function(request, status,  error){
        	...
        	alertMsgByJson(request);//alert(JSON.stringify(error));	
        }
    });
}
...
function renderTableBody(json){
	...
	for (var i = 0; i < listLength; i++) {
	    ...
	    bodyHtml += "<option value='I'"+(status == 'I' ? " selected": "")+">진행중</option>";
	...
	$('.tdRow').click(function(){
		...
		$('#modForm .status').html((status == 'I' ? "진행중": "완료됨"));	
...
function setCheckBoxWithModal(todoId){
	...
	$.ajax({
        ...
        cache    : false,
        success  : function( json, status, request ) {
        	//renderTableBody(JSON.stringify(json));
